...
- Add a connectivity-aware decoder (`ScaffoldConnectivityLogitsProcessor`) that
  ...
  RL sampling loop), works with multinomial, greedy and beam search, and needs
  no retraining.
...
- Keep publication manual through the `release` action, using PyPI Trusted
  Publishing (OpenID Connect) with PEP 740 attestations and no API token.
